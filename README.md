<a name="readme-top"></a>

<div align="center">
  
[![LinkedIn][linkedin-shield]][linkedin-url]

<br />
<a href="https://alinafedoriv-farmers-market.netlify.app/" target="_blank">
    <img src="assets/favicon.ico" alt="Logo" width="80" height="80">
</a>

<br />
<h3 align="center">The Yaremche Farmers Market</h3>

<br />
<p align="center">
    This repository houses the code and the assets used to build the Yaremche Farmers Market website.
<br />
  You can view a live demo of the project or watch the video demo below.
<br />
<br />
<a href="https://alinafedoriv-farmers-market.netlify.app/" target="_blank">View Live Demo</a>
</p>
</div>


<br />
<details>
  <summary>Table of Contents</summary>
  <ol>
    <li>
      <a href="#about-the-project">About The Project</a>
      <ul>
        <li><a href="#introduction">Introduction</a></li>
        <li><a href="#built-with">Built With</a></li>
      </ul>
    </li>
    <li><a href="#contact">Contact</a></li>
    <li><a href="#acknowledgments">Acknowledgments</a></li>
  </ol>
</details>

## About The Project

<br />

[![Typing SVG](https://readme-typing-svg.herokuapp.com?color=%212529&lines=The+Yaremche+Farmers+Market)](https://git.io/typing-svg)

https://github.com/afedoriv/the-yaremche-farmers-market/assets/99702784/21c01e2b-958d-4a79-b58c-724f529221b7

<br />

### Introduction

This repository houses the code and the assets used to build the Yaremche Farmers Market website.

On the webpage the users can find general information about the farmers market and it's location, the subscription options that are available and more. The users can contact the sponsor via email or by using the contact form as well as to submit a participation request. 

The website was built using HTML, Bootstrap and Formspree forms. And it is a great example of how limited tools can be used to create simple and yet beautiful websites.

<p align="right">(<a href="#readme-top">back to top</a>)</p>

### Built With

-  [![Bootstrap][Bootstrap.com]][Bootstrap-url]
-  ![HTML5](https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white)

<p align="right">(<a href="#readme-top">back to top</a>)</p>

## Contact

Alina Fedoriv - fedoriv2023@gmail.com

Project Link - [https://github.com/afedoriv/the-yaremche-farmers-market.git](https://github.com/afedoriv/the-yaremche-farmers-market.git)

<p align="right">(<a href="#readme-top">back to top</a>)</p>

## Acknowledgments

-   [Formspree](https://formspree.io/)
-   [Netlify](https://www.netlify.com/)
-   [Coverr](https://coverr.co/)
-   [Unsplash](https://unsplash.com/)
-   [Pexels](https://www.pexels.com/)

<p align="right">(<a href="#readme-top">back to top</a>)</p>

[linkedin-shield]: https://img.shields.io/badge/-LinkedIn-black.svg?style=for-the-badge&logo=linkedin&colorB=555
[linkedin-url]: https://www.linkedin.com/in/alina-fedoriv-89868720b/
[website-demo]: assets/website-demo.png
[Bootstrap.com]: https://img.shields.io/badge/Bootstrap-563D7C?style=for-the-badge&logo=bootstrap&logoColor=white
[Bootstrap-url]: https://getbootstrap.com
